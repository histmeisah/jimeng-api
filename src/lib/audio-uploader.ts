 ...
 * 音频使用与视频相同的 VOD 上传流程（FileType=video），返回 VideoMeta
 */
...
export async function uploadAudioBuffer(
  ...
  try {
    ...
    // 第二步：申请上传权限 (ApplyUploadInner)
    // 音频使用 FileType=video（VOD 不支持 FileType=audio）
    const now = new Date();
    ...
    const videoMeta = result.VideoMeta; // 即使是音频，VOD 也返回 VideoMeta
    ...
    if (videoMeta?.Duration && videoMeta.Duration > MAX_AUDIO_DURATION) {
      throw new Error(`音频时长 ${videoMeta.Duration.toFixed(2)}s 超过限制 (最大 ${MAX_AUDIO_DURATION}s)`);
    }

    logger.info(`音频上传完成: vid=${result.Vid}, duration=${videoMeta?.Duration}s, format=${videoMeta?.Format}`);
